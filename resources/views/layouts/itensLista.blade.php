
<li class="list-group-item d-flex justify-content-between align-items-center">
    <input hidden class="form-control w-25 inputEditar" name="item_1" type="text" value="{{ $dados->item_1 }}">

    <span class=" textEditar"><b class="p-1 alert alert-primary me-2">Item 1:</b>{{ $dados->item_1 }}</span>

    <span>
        <a class="btn btn-info btnEditar">
            <i class="fas fa-edit"></i>
        </a>
    </span>
</li>

<li class="list-group-item d-flex justify-content-between align-items-center">
    <input hidden class="form-control w-25 inputEditar" name="item_2" type="text" value="{{ $dados->item_2 }}">

    <span class=" textEditar"><b class="p-1 alert alert-primary me-2">Item 2:</b>{{ $dados->item_2 }}</span>

    <span>
        <a class="btn btn-info btnEditar">
            <i class="fas fa-edit"></i>
        </a>
    </span>
</li>

<li class="list-group-item d-flex justify-content-between align-items-center">
    <input hidden class="form-control w-25 inputEditar" name="item_3" type="text" value="{{ $dados->item_3 }}">

    <span class=" textEditar"><b class="p-1 alert alert-primary me-2">Item 3:</b>{{ $dados->item_3 }}</span>

    <span>
        <a class="btn btn-info btnEditar">
            <i class="fas fa-edit"></i>
        </a>
    </span>
</li>

<li class="list-group-item d-flex justify-content-between align-items-center">
    <input hidden class="form-control w-25 inputEditar" name="item_4" type="text" value="{{ $dados->item_4 }}">

    <span class=" textEditar"><b class="p-1 alert alert-primary me-2">Item 4:</b>{{ $dados->item_4 }}</span>

    <span>
        <a class="btn btn-info btnEditar">
            <i class="fas fa-edit"></i>
        </a>
    </span>
</li>

<li class="list-group-item d-flex justify-content-between align-items-center">
    <input hidden class="form-control w-25 inputEditar" name="item_5" type="text" value="{{ $dados->item_5 }}">

    <span class=" textEditar"><b class="p-1 alert alert-primary me-2">Item 5:</b>{{ $dados->item_5 }}</span>

    <span>
        <a class="btn btn-info btnEditar">
            <i class="fas fa-edit"></i>
        </a>
    </span>
</li>